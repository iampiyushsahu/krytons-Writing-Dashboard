...
$id = $_GET['id'];
$sql = "SELECT * from krytons_content where id = '$id'";
...
<?php
    ...
    ?>
        ...
        $a = "UPDATE krytons_content SET `topic_name` = '$topic_name', `write` = '$write', `images` = '$z' WHERE id = $id";
        $result = mysqli_query($conn, $a);
        if ($result) {
        ?>
            <script>
                alert("Done!");
                window.location.replace('view_table.php');
            </script>
            <?php
        ...
        ?>
            <script>
                alert("Error!" + <?php echo json_encode(mysqli_error($conn)); ?>);
            </script>
<?php
        }
    }
    ...
    if($flag == true){
        ...
        $aa = mysqli_fetch_assoc($result1);
        $s = $aa['images'];
        $a = "UPDATE krytons_content SET `topic_name` = '$topic_name', `write` = '$write', `images` = '$s' WHERE id = $id";
        $result = mysqli_query($conn, $a);
        if ($result) {
        ?>
            <script>
            ...
            <div class="container con">
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="topic_name1" id="formGroupExampleInput2" value="<?php echo $aa['topic_name'] ?>" placeholder="Write Topic Name Here">
                    </div>
                    <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label form_label">Write Topic</label>
                        <textarea id="editor" class="form-control" rows="50" cols="50" name="write1"><?php echo $aa['write'] ?></textarea>
                    </div>

                    <div class="mb-3">
                        <label for="formGroupExampleInput2" class="form-label form_label">Upload Picture</label>
                        <input type="file" class="form-control" name="file1" id="formGroupExampleInput2" value="<?php echo $aa['images'] ?>" placeholder="Picture">
                    </div>

                    <div class="mb-3">
                        <button type="submit" class="b1" name="book2">Submit</button>
                    </div>
                </form>
                <?php
                // }
